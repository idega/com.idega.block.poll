...
public class PollAdminWindow extends IWAdminWindow {
	...
	private void savePoll(IWContext iwc) {
		iwc.getIWMainApplication().getIWCacheManager().invalidateCacheWithPartialKey(Poll2.CACHE_KEY + this._pollID, Poll2.CACHE_KEY + this._pollID);
		...
		PollBusiness.setPollID(iwc, this._newObjInst, this._pollID);
		PollBusiness.setPollQuestionID(iwc, this._pollID, this._pollQuestionID);
		setParentToReload();
		close();
	}
